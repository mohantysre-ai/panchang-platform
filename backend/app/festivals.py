...
def _previous_full_moon(dt):
    step=timedelta(hours=6);cur=dt;prev,_=_phase(cur)
    for _ in range(70):
        nxt=cur-step;angle,_=_phase(nxt)
        if angle<180<=prev:
            a,b=nxt,cur
            for _ in range(28):
...
def _lunar_month(dt,system="Amanta"):
    full=_previous_full_moon(dt)
    if not full:return None
    _,sun=_phase(full);amanta=FULL_MOON_SIGN_TO_MONTH[int(sun//30)]
    angle,_=_phase(dt);tithi=int(angle//12)+1;month=amanta
...
def lunar_month_for_date(target,state_code,lat,lon,timezone):
    sunrise,_=rise_set(target,lat,lon,timezone)
    dt=sunrise or datetime(target.year,target.month,target.day,6,tzinfo=ZoneInfo(timezone))
    cfg="Amanta"
    # Regional configuration is intentionally kept lightweight here; Purnimanta regions are adjusted by festival rules.
    result=_lunar_month(dt,cfg) or {"index":None,"name":None,"system":cfg}
    result.update({"date":target.isoformat(),"state_code":state_code.upper(),"timezone":timezone})
    return result
def _sunrise_tithi(target,lat,lon,timezone):
    sunrise,_=rise_set(target,lat,lon,timezone)
    dt=sunrise or datetime(target.year,target.month,target.day,6,tzinfo=ZoneInfo(timezone))
    angle,_=_phase(dt);idx=int(angle//12)+1
    return idx,"Shukla" if idx<=15 else "Krishna",dt
def _solar_ingress_date(target,longitude,timezone):
    tz=ZoneInfo(timezone)
    for delta in range(-3,4):
        d=target+timedelta(days=delta);noon=datetime(d.year,d.month,d.day,12,tzinfo=tz);before,_=_phase(noon-timedelta(hours=12));after,_=_phase(noon+timedelta(hours=12))
        # compare sidereal solar longitude with wrap-aware crossing
        sb=before[0] if isinstance(before,tuple) else None
        # phase() returns angle,sun; recompute sun directly
        sun_b=sidereal_longitudes(noon-timedelta(hours=12))[0];sun_a=sidereal_longitudes(noon+timedelta(hours=12))[0]
...
def resolve_festivals(target,state_code,lat,lon,timezone):
    events=[_event(d,target) for d in _load_definitions() if _applies(d,state_code) and _matches(d,target,lat,lon,timezone,state_code)]
    events.sort(key=lambda x:(x["category"]!="major",x["id"]))
    return {"date":target.isoformat(),"state_code":state_code.upper(),"timezone":timezone,"events":events,"engine":"Swiss Ephemeris + sunrise tithi + astronomical lunar-month resolver"}
def festivals_for_date(target,state_code,lat,lon,timezone):
    key=f"festivals:v2:{target}:{state_code.upper()}:{lat:.5f}:{lon:.5f}:{timezone}"
    return get_or_create(key,"festivals",lambda:resolve_festivals(target,state_code,lat,lon,timezone))
