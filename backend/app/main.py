...
def health():
    return {"status":"ok","service":settings.app_name,"cache":cache.status(),"json_storage":settings.json_storage_enabled,"postgres":settings.postgres_enabled,"gemini_configured":bool(settings.gemini_api_key)}
...
def rashifal(lang:str=Query("kn"),date_str:str|None=Query(None)):
    lang=lang.lower(); lang=lang if lang in LANGUAGES else "kn"; target=date.fromisoformat(date_str) if date_str else current_date(settings.default_timezone)
    return get_or_create(f"rashifal:{target}:{lang}","rashifal",lambda:generate_rashifal(lang,target))
...
def festivals(state_code:str=Query("KA"),lat:float=Query(settings.default_lat),lon:float=Query(settings.default_lon),timezone:str=Query(settings.default_timezone),date_str:str|None=Query(None),year:int|None=Query(None),month:int|None=Query(None)):
    ...
    if year is not None or month is not None:
        today=current_date(timezone); y=year or today.year; m=month or today.month; m=m if 1<=m<=12 else today.month
...
def home():
    ...
    html=html.replace('</head>','<link rel="stylesheet" href="/regional-ui.css?v=3"><script defer src="/regional-ui.js?v=3"></script></head>')
